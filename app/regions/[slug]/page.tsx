...
export default async function RegionPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.keywordBand}><p>{area.name}에서 많이 찾는 보일러 상담</p><div>{[`${area.name} 보일러 교체`,`${area.name} 보일러 가격`,`${area.name} 경동나비엔`,`${area.name} 귀뚜라미`,`${area.name} 린나이`,...area.neighborhoods.flatMap(n=>[`${n} 보일러 교체`,`${n} 보일러 설치`,`${n} 보일러 가격`])].map(k=><span key={k}>{k}</span>)}</div></section>
    <section className={styles.section}><p className={styles.kicker}>동네별 사진 견적</p><h2>{area.neighborhoods.join("·")}<br/>보일러 교체 안내</h2><div className={styles.grid}>{area.neighborhoods.map((n,index)=><article key={n}><b>{String(index+1).padStart(2,"0")}</b><h3>{n} 보일러 교체·설치</h3><p>{n} 아파트·빌라·주택의 기존 보일러 모델, 난방 평수, 연통과 배수구 사진을 확인해 귀뚜라미·경동나비엔·린나이 제품과 예상 설치 범위를 안내합니다.</p></article>)}</div></section>
