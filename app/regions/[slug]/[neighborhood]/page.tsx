...
export async function generateMetadata({params}:{params:Promise<{slug:string;neighborhood:string}>}):Promise<Metadata>{const {slug,neighborhood}=await params;const area=getServiceArea(slug);if(!area||!area.neighborhoods.includes(neighborhood))return{};const title=`${neighborhood} 보일러 교체 비용·설치 가격`;const description=`${area.province} ${area.name} ${neighborhood} 가스보일러 교체 비용과 설치 가격을 확인하세요. 귀뚜라미·경동나비엔·린나이 보일러와 콘덴싱 제품을 사진으로 비교합니다.`;return{title,description,robots:{index:false,follow:true},keywords:[`${neighborhood} 보일러`,`${neighborhood} 보일러 교체`,`${neighborhood} 보일러 설치`,`${neighborhood} 보일러 가격`,`${neighborhood} 보일러 교체 비용`,`${neighborhood} 귀뚜라미 보일러`,`${neighborhood} 경동나비엔 보일러`,`${neighborhood} 린나이 보일러`],alternates:{canonical:`/regions/${slug}/${neighborhood}`},openGraph:{title:`${title} | 로켓보일러`,description,url:`/regions/${slug}/${neighborhood}`,images:["/og.png"]}};}
