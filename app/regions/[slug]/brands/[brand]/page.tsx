...
export async function generateMetadata({params}:{params:Promise<{slug:string;brand:string}>}):Promise<Metadata>{const {slug,brand:brandSlug}=await params;const area=getServiceArea(slug),brand=getBoilerBrand(brandSlug);if(!area||!brand)return{};const region=`${area.province} ${area.name}`;const title=`${area.name} ${brand.name} 보일러 교체 가격·설치`;const description=`${region} ${brand.name} 가스보일러 교체 가격과 콘덴싱 설치 조건을 확인하세요. ${area.neighborhoods.join("·")} 등 ${area.name} 사진 견적을 안내합니다.`;return{title,description,keywords:[`${area.name} ${brand.name} 보일러`,`${area.name} ${brand.name} 보일러 교체`,`${area.name} ${brand.name} 보일러 가격`,`${area.name} ${brand.name} 콘덴싱 보일러`,...area.neighborhoods.map(n=>`${n} ${brand.name} 보일러`)],alternates:{canonical:`/regions/${slug}/brands/${brandSlug}`}};}
