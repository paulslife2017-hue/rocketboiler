...
export default function Operations({ session, view }: { session: string; view: 'orders' | 'inventory' }) {
  ...
  return <>
    ...
    <div className={s.metrics}><button><span>{view === 'orders' ? '불러온 상품 주문' : '등록 모델'}</span><strong>{data ? (view === 'orders' ? data.orders.length : products.length) : '—'}</strong><em>{view === 'orders' ? '상품주문번호 기준 · 최대 1,000건 표시' : '브랜드·모델별 관리'}</em></button><button data-accent="gold"><span>출고 반영 대기</span><strong>{data ? waiting.length : '—'}<small>건</small></strong><em>결제 완료 주문 · 재고 확인 필요</em></button><button><span>모델 연결 필요</span><strong>{data ? waiting.filter(order => !order.sku).length : '—'}<small>건</small></strong><em>모델을 연결해야 재고 예약에 반영</em></button><button data-accent="green"><span>보유 재고</span><strong>{data ? products.reduce((sum, item) => sum + item.on_hand, 0) : '—'}<small>대</small></strong><em>등록·입고·출고 반영 기준</em></button></div>
    {view === 'orders' ? <>
