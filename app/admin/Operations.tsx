...
export default function Operations({ session, view }: { session: string; view: 'orders' | 'inventory' }) {
  ...
  return <>
    ...
    {view === 'inventory' ? <div className={`${s.metrics} ${s.inventoryMetrics}`}><article><span>관리 중인 모델</span><strong>{data ? products.length : '—'}<small>종</small></strong><em>수량을 등록한 보일러</em></article><article data-accent="gold"><span>총 보유 재고</span><strong>{data ? products.reduce((sum,item)=>sum+item.on_hand,0) : '—'}<small>대</small></strong><em>현재 창고에 보유한 수량</em></article><article data-accent="green"><span>주문 예약</span><strong>{data ? products.reduce((sum,item)=>sum+item.reserved,0) : '—'}<small>대</small></strong><em>모델이 연결된 미출고 주문</em></article></div> : <div className={s.metrics}><article><span>{view === 'orders' ? '불러온 상품 주문' : '등록 모델'}</span><strong>{data ? (view === 'orders' ? data.orders.length : products.length) : '—'}</strong><em>{view === 'orders' ? '상품주문번호 기준 · 최대 1,000건 표시' : '브랜드·모델별 관리'}</em></article><article data-accent="gold"><span>출고 반영 대기</span><strong>{data ? waiting.length : '—'}<small>건</small></strong><em>결제 완료 주문 · 재고 확인 필요</em></article><article><span>모델 연결 필요</span><strong>{data ? waiting.filter(order => !order.sku).length : '—'}<small>건</small></strong><em>모델을 연결해야 재고 예약에 반영</em></article><article data-accent="green"><span>보유 재고</span><strong>{data ? products.reduce((sum, item) => sum + item.on_hand, 0) : '—'}<small>대</small></strong><em>등록·입고·출고 반영 기준</em></article></div>}
    {view === 'orders' ? <>
      ...
      <section className={s.panel}><div className={s.sectionTitle}><h2>스마트스토어 주문</h2><input className={s.orderSearch} aria-label="주문 검색" placeholder="주문번호 · 고객 · 상품 검색" value={search} onChange={event => setSearch(event.target.value)} /></div><div className={s.tableScroll}><table className={`${s.table} ${s.mobileCards} ${s.orderCards}`}><thead><tr><th>상품주문번호·고객</th><th>상품·옵션</th><th>수량</th><th>네이버 상태</th><th>재고 모델 연결</th><th>재고 반영</th></tr></thead><tbody>{orders.map(order => <tr key={order.id}><td data-label="고객·주문번호"><b>{order.customer || '고객명 없음'}</b><small>{order.id}</small><small>{order.phone}</small></td><td data-label="상품·옵션"><span className={s.productName}>{order.product}</span><small className={s.productName}>{order.option_name}</small><small>{order.ordered_at.slice(0,10)}</small></td><td data-label="수량">{order.quantity}</td><td data-label="주문 상태"><span className={s.badge}>{names[order.status] || order.status}</span></td><td data-label="재고 모델 연결"><select aria-label={`${order.id} 재고 모델`} disabled={busy || order.dispatched} value={order.sku || ''} onChange={event => { if(event.target.value) void perform({ action: 'map', id: order.id, sku: event.target.value }); }}><option value="">모델 선택</option>{products.map(product => <option key={product.sku} value={product.sku}>{product.model} ({product.sku})</option>)}</select></td><td data-label="재고 반영">{order.dispatched ? cancelled.includes(order.status) ? <button className={s.ghost} disabled={busy} onClick={() => perform({ action: 'return', id: order.id })}>회수 재고 반영</button> : <span className={s.badge} data-status="completed">출고 반영됨</span> : !cancelled.includes(order.status) && order.status !== 'PAYMENT_WAITING' ? <button className={s.primary} disabled={busy || !order.sku} onClick={() => perform({ action: 'dispatch', id: order.id })}>출고 반영</button> : <span>예약 제외</span>}</td></tr>)}</tbody></table></div>{!orders.length && <div className={s.empty}><span>N</span><h3>{data?.naverConnected ? '불러온 주문이 없습니다' : '스마트스토어를 연결해 주세요'}</h3><p>API 연결 후 날짜를 선택하면 주문을 이곳에서 함께 볼 수 있습니다.</p></div>}<p className={s.hint}>실제 제품을 내보낸 뒤 ‘출고 반영’을 누르면 보유 재고가 차감됩니다. 취소·반품된 출고 건은 실물 회수 후 ‘회수 재고 반영’을 눌러 주세요.</p></section>
    </> : <>
      <InventoryManager products={products} loaded={!!data} busy={busy} perform={perform} />
