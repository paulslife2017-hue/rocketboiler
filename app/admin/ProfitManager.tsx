...
type Data={prices:{sku:string;sale:number;cost:number|null;cost_source:string}[];records:ProfitRecord[];totals:{count:number;revenue:string;profit:string;missing:number;estimates:number}};
...
export default function ProfitManager({session}:{session:string}) {
  ...
  async function save(action:'price'|'record') {
    ...
    try {
      const response=await fetch('/api/admin/profit',{method:'POST',headers:{authorization:`Bearer ${session}`,'content-type':'application/json'},body:JSON.stringify({action,...row,sku,cost_source:costSource})});
  ...
  function choose(value:string){setSku(value);const model=profitCatalog.find(item=>item.sku===value);const prices=data?.prices.find(item=>item.sku===value);setCostSource(prices?.cost_source??'');setRow({...newRow(),model:model?`${model.brand} ${model.model}`:'',sale:prices?.sale??referencePrices[value]??0,cost:prices?.cost??null});setMessage('');}
  ...
  return <div className={s.profitWorkspace}>
    ...
    <section className={s.panel}><div className={s.sectionTitle}><h2>설치 건별 수익 계산</h2><button className={s.ghost} disabled={busy} onClick={()=>{setRow(newRow());setSku('');setCostSource('');setMessage('');}}>+ 새 기록</button></div>
      ...
      <p className={s.hint}>저장된 단가가 없으면 2026-08-21 사진의 경동·귀뚜라미·린나이 견적표의 금액을 판매 참고가로 표시합니다. 매입 원가표는 아니며 실제 판매 조건과 모델 표기를 확인한 뒤 수정하세요. 매입 참고가는 2026-08-22 첨부 단가표의 브랜드·종류·용량 기준으로 연결했습니다. 정확한 모델명이 없는 자료이므로 적용 모델과 부가세·포함 품목을 확인하세요. 용량이 일치하지 않는 모델은 원가를 비워 두었습니다.</p>
      {costSource&&<p className={s.notice}>원가 출처: {costSource} · 모델 확인 전 참고 계산입니다.</p>}
      {row&&<form onSubmit={event=>{event.preventDefault();void save('record');}}><div className={s.dateInputs}>
        ...
        <label className={s.field}>제품 매입 단가 · 원<input type="number" min="0" max="100000000" step="1" placeholder="원가 미확인" disabled={busy} value={row.cost??''} onChange={event=>{setRow({...row,cost:event.target.value===''?null:Number(event.target.value)});setCostSource('직접 입력');}}/></label>
