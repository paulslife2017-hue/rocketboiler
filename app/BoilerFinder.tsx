...
function recommendation(areaText: string, drain: string, brand: string, controllers: string, homeType: string, boilerPosition: string) {
  ...
  const conditions = [controllers === "2개 이상" ? "각방제어 호환 장치 검토" : "", homeType === "복도식 아파트" ? "복도식 아파트 설치 조건" : "", boilerPosition === "난방 바닥보다 아래" ? "상향식 설치 조건" : ""].filter(Boolean);
