...
function environmentCredentials():Credentials|null{const customerId=process.env.NAVER_ROCKET_AD_CUSTOMER_ID,apiKey=process.env.NAVER_ROCKET_AD_API_KEY,secretKey=process.env.NAVER_ROCKET_AD_SECRET_KEY;return customerId&&apiKey&&secretKey?{customerId,apiKey,secretKey}:null;}
async function credentials(sql:ReturnType<typeof database>){const rows=await sql`SELECT encrypted FROM boiler_integrations WHERE name='naver_searchads'`;return rows.length?decrypt(rows[0].encrypted):environmentCredentials();}
...
export async function POST(r:NextRequest){
  ...
  try{const b=await r.json();
    if(b.action==='campaign-toggle'){
      if(typeof b.id!=='string'||!/^cmp-[\w-]+$/.test(b.id)||typeof b.paused!=='boolean'||typeof b.expectedPaused!=='boolean')return NextResponse.json({error:'캠페인과 ON/OFF 설정을 확인해 주세요.'},{status:400});
      await initialize();const sql=database();const c=await credentials(sql);if(!c)return NextResponse.json({error:'광고 계정 연결이 필요합니다.'},{status:409});const scope=await siteGroups(c);
      if(!scope.groups.some(g=>g.nccCampaignId===b.id))return NextResponse.json({error:'로켓보일러 사이트에 연결된 캠페인만 변경할 수 있습니다.'},{status:403});
      const campaign=await adsRequest(c,'/ncc/campaigns/'+b.id);if(campaign.userLock!==b.expectedPaused)return NextResponse.json({error:'광고 설정이 변경됐습니다. 새로 조회한 뒤 다시 선택해 주세요.'},{status:409});
      return NextResponse.json({ok:true,...await setCampaignLock(c,campaign,b.paused)});
    }
    if(b.action==='toggle'){
    ...
    if(b.action==='optimize'){
      await initialize();const sql=database();const c=await credentials(sql);if(!c)return NextResponse.json({error:'광고 계정 연결이 필요합니다.'},{status:409});const scope=await siteGroups(c);const result=await optimizeBids(c,scope.groups,b.dryRun!==false);return NextResponse.json({ok:true,result});
    }
