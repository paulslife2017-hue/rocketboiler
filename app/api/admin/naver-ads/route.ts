...
import {campaigns,optimizeBids,setGroupLock,siteGroups,SITE_HOST,decrypt,encrypt,request as adsRequest,statsOf,validPeriod,type Credentials} from './client';
...
export async function GET(r:NextRequest){
  ...
  try{await initialize();const sql=database();const c=await credentials(sql);if(!c)return NextResponse.json({connected:false},{headers:{'Cache-Control':'private, no-store'}});
    const scope=await siteGroups(c);const list=scope.groups;
