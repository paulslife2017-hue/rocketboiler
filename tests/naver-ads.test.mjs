...
test('mobile rank bid changes are capped and step-limited',()=>{assert.equal(client.nextSafeBid(1000,3000),1200);assert.equal(client.nextSafeBid(3000,1000),2700);assert.equal(client.nextSafeBid(3400,5000),3500);assert.equal(client.nextSafeBid(1000,1080),1080);});
...
test('bizmoney balance validates amount and lock states',()=>{assert.deepEqual(JSON.parse(JSON.stringify(client.bizmoneyOf({bizmoney:123456,budgetLock:false,refundLock:false}))),{balance:123456,budgetLocked:false,refundLocked:false});for(const value of [{},{bizmoney:-1,budgetLock:false,refundLock:false},{bizmoney:100,budgetLock:'false',refundLock:false}])assert.throws(()=>client.bizmoneyOf(value));});
